...
def result():
    ...
    if 'seq' in request_json:
        addr = request_json['ID']
        transformed_string = transform_data(request_json, addr)
        if transformed_string == None:
            return get_Time_as_JSON()
...
def resolve_commands(request):
    ...
    if command == 'inc_data':
        ...
        for entry in corrector.entries:
            string = string + "Device: " + entry.id_d + " | Ideal_Seconds: " + str(
                entry.ideal_seconds) + "  | DevEUI: " + str(entry.devEui) + "\n"
        ...
        for entry in last_entry_dict:
            string = string + "Device: " + entry + " | Counter: " + str(
                last_entry_dict[entry]['counter']) + " | Seq: " + str(
                last_entry_dict[entry]['sequence']) + " | kW: " + str(last_entry_dict[entry]['kW']) + "\n"
        return string
...
def transform_data(data_sensor, addr):
    global devices
    global last_entry_dict
    global margin_devices
    ...
    if devices[deveui]['dt'] == 'Arduino-PWR':
        dateTime = datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%f+00:00')
        id = devices[deveui]['kd']
        seq = int(data_sensor['seq'])
        if seq == last_entry_dict[id]['sequence']:
            kW = float(data_sensor["kWh"]) - last_entry_dict[id]['kW']
            alert.append(("Repeated Sequence", "Count"))
        else:
            kW = float(data_sensor["kWh"])
        last_entry_dict[id]['kW'] = float(data_sensor["kWh"])
        last_entry_dict[id]['sequence'] = int(data_sensor["seq"])
...
def start_runner():
    def start_loop():
        ...
        while not_started:  #
            try:
                url = 'http://192.168.1.108:5005/receivedata/'
...
def get_Time_as_JSON():
    hour = datetime.datetime.now().hour
    minute = datetime.datetime.now().minute
    second = datetime.datetime.now().second
    return '{"hour":'+str(hour)+', "min":'+str(minute)+', "sec":'+str(second)+'}'


if __name__ == '__main__':
    start_runner()
    app.run(host='192.168.1.108', debug=True, port=5005)  # 10.58.20.35